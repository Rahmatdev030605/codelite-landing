...
<div class="col-md-16">
    ...
    <div class="card-header d-flex justify-content-between">
            <h3 class="card-video"><strong>LEADERSHIPS</strong></h3>
            <a href="{{ route('admin.new-leadership') }}" class="btn btn-success" style="color: white; width: 10%;">New +</a>
        </div>
        ...
        <div class="card-body">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-5">
                            <div class="table-responsive">
                                <form action="{{ route('admin.show-leadership') }}" method="GET">
                        ...
                        <div class="col-md-6">
                            <form id="filterForm" action="{{ route('admin.show-leadership') }}" method="GET" class="filter">
                                @csrf
                    ...
                    <div class="table-responsive">
                        <table class="table table-transparent table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $leader->title }}</td>
                                    <td>{{ $leader->heading }}</td>
                                    <td>{{ $leader->sub_heading }}</td>
                                    <td style="font-size: 12px;">{{ $leader->created_at }}</td>
                                    <td style="font-size: 12px;">{{ $leader->updated_at }}</td>
                                    <td style="text-align: center;">
                                        <span class="{{ $leader->status == 'active' ? 'text-success' : 'text-danger' }}" style="font-size: 14px">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.edit-leadership', ['id' => $leader->id]) }}" class=" btn-sm edit-btn btn-primary btn-animate">
                                            <i class="cil-pencil"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <button class="btn-sm btn-danger delete-btn" data-partner-id="{{ $leader->id }}" data-toggle="modal" data-target="#deleteContent{{ $leader->id }}">
                                            <i class="cil-trash"></i>
                                        </button>
                                    </td>
                                </tr>
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div style="font-size: 13px">
                    <p>Total: {{ $totalLeader }} data</p>
                </div>
            </div>


            @foreach($leaders as $leader)
            <div class="modal fade" id="deleteContent{{ $leader->id }}" tabindex="-1" aria-labelledby="deleteContentModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-m">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-center">
                            <form id="content-delete" method="POST" action="{{ route('admin.delete-leadership', $leader->id) }}">
                                @csrf
                                @method('DELETE')
                                <p>Are you sure you want to delete <strong id="partner-name">{{ $leader->heading }}</strong>?</p>
