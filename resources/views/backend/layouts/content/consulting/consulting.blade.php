...
<div class="col-md-16">
    ...
    <div class="card-header d-flex justify-content-between">
            <h3 class="card-video"><strong>CONSULTING</strong></h3>
            <a href="{{ route('admin.new-consulting') }}" class="btn btn-success" style="color: white; width: 10%;">New +</a>
        </div>
        ...
        <div class="card-body">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-5">
                            <div class="table-responsive">
                                <form action="{{ route('admin.show-consulting') }}" method="GET">
                        ...
                        <div class="col-md-6">
                            <form id="filterForm" action="{{ route('admin.show-consulting') }}" method="GET" class="filter">
                                @csrf
                ...
                <div class="container-fluid">
                    <div class="table-responsive">
                        <table class="table table-transparent table-striped">
                            ...
                            <tbody>
                                @foreach($consulting as $consult)
                                <tr>
                                    <td>{{ $consult->title }}</td>
                                    <td>{{ $consult->heading }}</td>
                                    <td>{{ $consult->sub_heading }}</td>
                                    <td style="font-size: 12px;">{{ $consult->created_at }}</td>
                                    <td style="font-size: 12px;">{{ $consult->updated_at }}</td>
                                    <td style="text-align: center;">
                                        <span class="{{ $consult->status == 'active' ? 'text-success' : 'text-danger' }}" style="font-size: 14px">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.edit-consulting', ['id' => $consult->id]) }}" class=" btn-sm edit-btn btn-primary btn-animate">
                                            <i class="cil-pencil"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <button class="btn-sm btn-danger delete-btn" data-partner-id="{{ $consult->id }}" data-toggle="modal" data-target="#deleteContent{{ $consult->id }}">
                                            <i class="cil-trash"></i>
                                        </button>
                                    </td>
                                </tr>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    {{ $consulting->links('pagination::bootstrap-4') }}
                </div>
                <div style="font-size: 13px">
                    <p>Total: {{ $totalConsult }} data</p>
                </div>
            </div>


            @foreach($consulting as $consult)
            <div class="modal fade" id="deleteContent{{ $consult->id }}" tabindex="-1" aria-labelledby="deleteContentModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-m">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-center">
                            <form id="content-delete" method="POST" action="{{ route('admin.delete-consulting', $consult->id) }}">
                                @csrf
                                @method('DELETE')
                                <p>Are you sure you want to delete <strong id="partner-name">{{ $consult->heading }}</strong>?</p>
