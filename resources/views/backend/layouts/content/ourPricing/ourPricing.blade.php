...
<div class="col-md-16">
    ...
    <div class="card-header d-flex justify-content-between">
            <h3 class="card-video"><strong>OUR PRICING</strong></h3>
            <a href="{{ route('admin.new-our-pricing') }}" class="btn btn-success" style="color: white; width: 10%;">New +</a>
        </div>
        ...
        <div class="card-body">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-5">
                            <div class="table-responsive">
                                <form action="{{ route('admin.show-our-pricing') }}" method="GET">
                        ...
                        <div class="col-md-6">
                            <form id="filterForm" action="{{ route('admin.show-our-pricing') }}" method="GET" class="filter">
                                @csrf
                        ...
                        <table class="table table-transparent table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $our->title }}</td>
                                    <td>{{ $our->heading }}</td>
                                    <td>{{ $our->sub_heading }}</td>
                                    <td><img src="{{ asset($our->image) }}" alt="Image not found" width="200" height="100"></td>
                                    <td style="font-size: 12px;">{{ $our->created_at }}</td>
                                    <td style="font-size: 12px;">{{ $our->updated_at }}</td>
                                    <td style="text-align: center;">
                                        <span class="{{ $our->status == 'active' ? 'text-success' : 'text-danger' }}" style="font-size: 14px">
                                            @if($our->status == 'active')
                                    ...
                                    <td>
                                        <a href="{{ route('admin.edit-our-pricing', ['id' => $our->id]) }}" class=" btn-sm edit-btn btn-primary btn-animate">
                                            <i class="cil-pencil"></i>
                                        </a>
                                    </td>
                                    <td>
                                        <button class="btn-sm btn-danger delete-btn" data-partner-id="{{ $our->id }}" data-toggle="modal" data-target="#deleteContent{{ $our->id }}">
                                            <i class="cil-trash"></i>
                                        </button>
                                    </td>
                                </tr>
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    {{ $ours->links('pagination::bootstrap-4') }}
                </div>
                <div style="font-size: 13px">
                    <p>Total: {{ $totalOur }} data</p>
                </div>
            </div>


            @foreach($ours as $our)
            <div class="modal fade" id="deleteContent{{ $our->id }}" tabindex="-1" aria-labelledby="deleteContentModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-m">
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-center">
                            <form id="content-delete" method="POST" action="{{ route('admin.delete-our-pricing', $our->id) }}">
                                @csrf
                                @method('DELETE')
                                <p>Are you sure you want to delete <strong id="partner-name">{{ $our->heading }}</strong>?</p>
